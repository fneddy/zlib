...
local once_t power_crc32_made = ONCE_INIT;
local void power_crc32_setup() {
    if (__builtin_cpu_supports("arch_2_07"))
        crc32_z_hook = _crc32_z_power8;
    else
        crc32_z_hook = crc32_z;
}
local unsigned long power_crc32_init(unsigned long crc, const unsigned char FAR *buf, z_size_t len)
{
    once(&power_crc32_made,power_crc32_setup);
    return crc32_z_hook(crc, buf, len);
}

ZLIB_INTERNAL unsigned long (*crc32_z_hook)(unsigned long crc, const unsigned char FAR *buf, z_size_t len) = power_crc32_init;
